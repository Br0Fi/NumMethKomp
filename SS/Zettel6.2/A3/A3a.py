...
for i in range(N_t):
    ...
    if (i % plotEveryNth == 0):
        c = np.fft.ifft2(ck)
        plt.suptitle("$L=$"+str(int(Lx))+", $N=$"+str(Nx)+", $\\varepsilon=$"+str(epsilon)+", $\\delta=$"+str(delta)+", $k=$"+str(k)+", $\\varphi=$"+str(round(rd, 5)))
        plt.subplot(1,2,1)
        plt.cla()
        plt.imshow(c.real)
        ...
        filename = "/A3a-2D{:04d}.png".format(round(i/plotEveryNth))
